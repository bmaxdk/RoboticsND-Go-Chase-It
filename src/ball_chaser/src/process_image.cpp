...
void process_image_callback(const sensor_msgs::Image img)
{
    ...
    // Threshold for a valid white ball detection
    int white_ball_threshold = 50;

    // Loop through each pixel in the image and calculate the mean position of the white pixels
    for (int i = 0; i < img.height * img.step; i += 3) {
        int current_index = (i % img.step) / 3;

        if (img.data[i] == white_pixel && img.data[i + 1] == white_pixel && img.data[i + 2] == white_pixel) {
            mean_pos += current_index;
            white_pixel_count++;
        }
    }
    ...
    if (white_pixel_count > 0) {
        mean_pos /= white_pixel_count;
    }

    // Decide on robot motion
    if (white_pixel_count > white_ball_threshold) {
        if(mean_pos < img.width/3) {
            drive_robot(0.0, 0.5); // Move left
        }
        else if(mean_pos > 2*img.width/3) {
